...
class TestDevAgentGeneratesInitialPlan:
    def test_generates_initial_plan_for_open_task(
        ...
        assert has_plan_heading, (
            f"Expected plan comment to contain Markdown heading. Bodies: {plan_bodies}"
        )

        # 3. Comment contains at least one technical section
        has_tech_section = any(
            any(kw in body.lower() for kw in ("approach", "steps", "implementation",
                                               "solution", "design", "architecture"))
            for body in plan_bodies
        )
        assert has_tech_section, (
            f"Expected plan to contain a technical section. Bodies: {plan_bodies}"
        )

        # 4. update_task NOT called with IN PROGRESS (BR-8)
        in_progress_calls = [
            t for t in transition_calls
            if "IN PROGRESS" in str(t).upper() or "in_progress" in str(t).lower()
        ]
        assert len(in_progress_calls) == 0, (
            f"BR-8 violated: Dev Agent transitioned to IN PROGRESS during planning. "
            f"Calls: {transition_calls}"
        )
        ...
        assert len(accepted_calls) == 0, (
            f"BR-1 violated: Dev Agent wrote ACCEPTED status. Calls: {transition_calls}"
        )

        # 6. update_task NOT called with IN PLANNING (BR-10)
        in_planning_calls = [
            t for t in transition_calls
            if "IN PLANNING" in str(t).upper() or "in_planning" in str(t).lower()
        ]
        assert len(in_planning_calls) == 0, (
        ...
        assert not stub.was_called("create_pull_request"), (
            "BR-8 violated: Dev Agent opened a PR during planning mode"
        )
...
class TestDevAgentBatchPlanning:
    def test_generates_plans_for_multiple_open_tasks(
        ...
        task_details = {
            "cu-30": {"id": "cu-30", "name": "Feature A", "status": {"status": "OPEN"},
                      "description": "Implement Feature A with caching layer."},
            "cu-31": {"id": "cu-31", "name": "Feature B", "status": {"status": "OPEN"},
                      "description": "Add Feature B as background worker."},
            "cu-32": {"id": "cu-32", "name": "Feature C", "status": {"status": "OPEN"},
                      "description": "Build Feature C API endpoints."},
        }

        stub.register_tool("get_task", lambda args: (
            task_details.get(args.get("task_id", ""), {"id": "unknown", "name": "Unknown",
                                                        "status": {"status": "OPEN"},
        ...
        stub.register_tool("search_tasks", lambda args: [
            {"id": tid, "name": d["name"], "status": {"status": "OPEN"},
        ...
        tracker_registry = _make_stub_tracker_registry(
            open_tickets=[
                TicketRecord(id=tid, source="clickup", title=d["name"],
                             url="", raw_status="OPEN")
                for tid, d in task_details.items()
            ],
        )
        ...
        assert not stub.was_called("create_pull_request"), (
            "BR-8 violated: Dev Agent opened a PR during batch planning"
        )
...
class TestDevAgentRevisesPlanOnHumanFeedback:
    def test_revises_plan_on_human_feedback(
        ...
        stub.register_tool("get_task", lambda args: {
            "id": "cu-21", "name": "DB migration tool",
        ...
        assert len(in_planning_calls) == 0, (
            f"BR-10 violated: Dev Agent wrote IN PLANNING. Calls: {transition_calls}"
        )
        ...
        assert len(accepted_calls) == 0, (
            f"BR-1 violated: Dev Agent wrote ACCEPTED. Calls: {transition_calls}"
        )
...
class TestPlanCommentIncludesHumanNotification:
    def test_plan_comment_includes_human_notification(
        ...
        assert len(add_comment_calls) >= 1, (
            "Expected Dev Agent to post plan comment."
        )
        ...
        assert has_notification, (
            f"Expected plan comment to include human notification phrase. Bodies: {plan_bodies}"
        )
...
class TestFullPlanningLoop:
    def test_full_planning_loop(
        ...
        stub.register_tool("get_task", lambda args: {
            "id": "cu-22", "name": "Search indexing service",
            "status": {"status": "OPEN"},
        ...
        stub.register_tool("create_pull_request", lambda args: (
            pr_calls.append(args) or {"html_url": "https://github.com/org/repo/pull/0", "number": 0}
        ))
        stub.register_tool("search_tasks", lambda args: [
            {"id": "cu-22", "name": "Search indexing service",
             "status": {"status": "OPEN"}, "url": "https://app.clickup.com/t/cu-22"},
        ])
        stub.register_tool("search_issues", lambda args: [])
        stub.register_tool("reply_to_thread", lambda args: {"ok": True})
        stub.register_tool("send_message", lambda args: {"ok": True})
        ...
        tracker_registry_run1 = _make_stub_tracker_registry(
            open_tickets=[
                TicketRecord(id="cu-22", source="clickup",
        ...
        # RUN 1 assertions
        assert len(add_comment_calls_run1) >= 1, (
            "Expected initial plan comment in run 1"
        )
        assert len(pr_calls) == 0, (
            "BR-8 violated: PR opened during planning run 1"
        )
        assert len(in_progress_calls) == 0, (
            "BR-8 violated: IN PROGRESS transition during planning run 1"
        )
        assert len(accepted_write_calls) == 0, (
            "BR-1 violated: ACCEPTED written during planning run 1"
        )
        ...
        tracker_registry_run2 = _make_stub_tracker_registry(
            in_planning_tickets=[
                TicketRecord(id="cu-22", source="clickup",
                             title="Search indexing service", url="", raw_status="IN PLANNING"),
            ],
        ...
        assert len(pr_calls) == 0, (
            "BR-8 violated: PR opened during planning run 2"
        )
        assert len(in_planning_write_calls) == 0, (
            "BR-10 violated: IN PLANNING written during planning run 2"
        )
        assert len(accepted_write_calls) == 0, (
            "BR-1 violated: ACCEPTED written during any planning run"
        )
