"""
E2E tests: Full dev lifecycle — ClickUp backend (E2E-20 through E2E-24).

Scenario steps covered: S1 → COMPLETE (all features, dev_agent only)

Complete journey:
  1. Supervisor @mentions dev in Slack thread → thread summary → task comment
  2. Human sets task to ACCEPTED (simulated by pre-seeding stub response)
  3. Scan job picks up ACCEPTED task → IN PROGRESS → PR opened → IN REVIEW
