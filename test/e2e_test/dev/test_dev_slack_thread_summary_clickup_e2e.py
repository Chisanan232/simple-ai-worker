...
class TestDevReadsThreadAndUpdatesTicket:
    def test_dev_reads_thread_and_updates_ticket(
        ...
        stub.register_tool(
            ...
            lambda args: {
                "ok": True,
                "permalink": "https://slack.com/archives/C001/p111222",
            },
        )
        stub.register_tool(
            "add_comment",
        ...
        assert stub.was_called("get_messages"), "Expected LLM to call slack/get_messages to fetch thread history"
        assert stub.was_called("add_comment"), "Expected LLM to call clickup/add_comment on the task"
...
class TestDevAsksForTicketWhenNoneFound:
    def test_dev_asks_for_ticket_when_none_found(
        ...
        stub.register_tool(
            ...
            lambda args: {
                "ok": True,
                "permalink": "https://slack.com/archives/C002/p222333",
            },
        )
        stub.register_tool("reply_to_thread", lambda args: {"ok": True})
        ...
        assert len(stub.calls_to("add_comment")) == 0, (
            f"add_comment must NOT be called when no ticket ID found (BR-6). " f"Got: {stub.calls_to('add_comment')}"
        )
...
class TestSlackPermalinkInComment:
    def test_permalink_in_ticket_comment(
        ...
        assert (
            "slack.com/archives" in all_comment_text
...
class TestNoNewTicketCreated:
    def test_no_new_ticket_created(
        ...
        stub.register_tool(
            ...
            lambda args: {
                "ok": True,
                "permalink": "https://slack.com/archives/C003/p333",
            },
        )
        ...
        assert len(create_calls) == 0, f"LLM must NOT create new tickets. Calls: {create_calls}"
...
class TestClickUpTaskUpdated:
    def test_clickup_task_updated_when_clickup_id_found(
                ...
                "messages": [
                    {
                        "user": "U1",
                        "text": (
                            "Please implement: " "https://app.clickup.com/t/abc123def456 " "(use async queue pattern)"
                        ),
                        "ts": "1.1",
                    },
                ],
        ...
        stub.register_tool(
            ...
            lambda args: {
                "ok": True,
                "permalink": "https://slack.com/archives/C004/p444",
            },
        )
        stub.register_tool("add_comment", _add_comment)
        stub.register_tool("reply_to_thread", lambda args: {"ok": True})
        ...
        assert len(add_comment_calls) > 0, "Expected LLM to call add_comment for the ClickUp task found in thread"
