...
class TestPlannerSurveysNewIdea:
    def test_e2e_pi_01_responds_without_creating_tickets(
        ...
        assert len(create_task_calls) == 0, (
            f"Planner must NOT create ClickUp tasks during initial idea discussion. "
            f"Got: {create_task_calls}"
        )
        assert len(create_issue_calls) == 0, (
            f"Planner must NOT create JIRA issues during initial idea discussion. "
...
class TestPlannerPostsSurveyPlan:
    def test_e2e_pi_02_survey_plan_posted_with_key_dimensions(
        ...
        stub.register_tool("get_messages", lambda args: {
            ...
            "messages": [
                {"user": "U1",
                 ...
                 "ts": "100.1"},
                {"user": "UBOT",
                 "text": "What type of restaurants? And what is the main goal?",
                 "ts": "100.2"},
                {"user": "U1",
                 "text": "Small to medium restaurants. Main goal is to reduce food waste.",
                 "ts": "100.3"},
                {"user": "UBOT",
                 "text": "Who would be the primary buyer?",
                 "ts": "100.4"},
                {"user": "U1",
        ...
        stub.register_tool("create_task", lambda args: (
            create_task_calls.append(args) or {"id": "cu-ERR"}
        ))
        ...
        assert dimension_hits >= 3, (
            f"Expected at least 3 key dimensions. Found {dimension_hits}. "
            f"Text (first 600): {all_reply_text[:600]}"
        )
        assert len(create_task_calls) == 0, (
...
class TestPlannerRejectsIdea:
    def test_e2e_pi_03_rejected_task_created_no_dev_lead(
        ...
        stub.register_tool("send_message", lambda args: (
            send_message_calls.append(args) or {"ok": True, "ts": "103.10"}
        ))
        stub.register_tool("get_messages", lambda args: {
            "ok": True,
            "messages": [
                {"user": "U1",
                 "text": "[planner] I want to build a restaurant inventory SaaS",
                 "ts": "100.1"},
                {"user": "UBOT",
                 "text": "### 📋 Idea Survey Plan\n**1. Marketing Value** ...",
                 "ts": "100.2"},
                {"user": "U1",
        ...
        _run_planner(
            message=(
                "[planner] Actually, let's drop this idea. "
                "The market is too competitive and we lack the budget."
            ),
            thread_ts="100.3",
            stub=stub,
            registry=registry,
        ...
        assert len(create_task_calls) >= 1, (
            f"Expected ClickUp task for rejected conclusion. Got: {create_task_calls}"
        )
        all_task_text = " ".join(str(c) for c in create_task_calls).upper()
        assert "REJECTED" in all_task_text, (
            f"Expected task to have REJECTED status. calls: {create_task_calls}"
        )

        dev_lead_in_send = any(
            "dev lead" in str(c).lower() or "[dev lead]" in str(c).lower()
            for c in send_message_calls
        )
        assert not dev_lead_in_send, (
            f"BR-12 violation: Dev Lead mentioned on reject path. calls: {send_message_calls}"
        )
...
class TestPlannerAcceptsIdea:
    def test_e2e_pi_04_open_task_and_dev_lead_handoff(
        ...
        stub.register_tool("send_message", lambda args: (
            send_message_calls.append(args) or {"ok": True, "ts": "104.10"}
        ))
        stub.register_tool("get_messages", lambda args: {
            "ok": True,
            "messages": [
                ...
                {"user": "UBOT",
                 ...
                 "ts": "100.2"},
                {"user": "U1",
                 "text": "Great plan! Let's proceed with the MVP. I approve this.",
                 "ts": "100.3"},
            ],
        })
        stub.register_tool("update_task", lambda args: {"ok": True})
        ...
        assert len(create_task_calls) >= 1, (
            f"Expected ClickUp task for accepted conclusion. Got: {create_task_calls}"
        )
        all_task_text = " ".join(str(c) for c in create_task_calls).upper()
        assert "OPEN" in all_task_text, (
            f"Expected OPEN task on acceptance. calls: {create_task_calls}"
        )
        ...
        assert dev_lead_in_send, (
            f"Expected [dev lead] mention in send_message. calls: {send_message_calls}"
        )
        assert len(accepted_status_writes) == 0, f"BR-1 violated: {accepted_status_writes}"
...
class TestFullPlannerDevLeadLifecycle:
    def test_e2e_pi_05_full_idea_lifecycle_planner_to_dev_lead(
        ...
        stub.register_tool("create_task", handle_create_task)
        stub.register_tool("reply_to_thread", lambda args: (
            reply_calls.append(args) or {"ok": True, "ts": "105.99"}
        ))
        stub.register_tool("send_message", lambda args: (
            send_message_calls.append(args) or {"ok": True, "ts": "105.100"}
        ))
        stub.register_tool("get_messages", lambda args: {
            "ok": True,
            "messages": [
                {"user": "U1",
                 "text": "[planner] I want to build an AI-powered meal planning app",
                 "ts": "105.1"},
                {"user": "UBOT",
                 "text": "Who is the primary user?",
                 "ts": "105.2"},
                {"user": "U1",
                 "text": "Parents with kids aged 5-16.",
                 "ts": "105.3"},
                {"user": "UBOT",
                 "text": (
                     "### 📋 Idea Survey Plan\n"
                 ...
                 "ts": "105.4"},
                {"user": "U1",
                 "text": "This looks great! Go ahead, let's do it. Approved.",
                 "ts": "105.5"},
            ],
        })
        stub.register_tool("update_task", lambda args: {"ok": True})
        ...
        assert len(create_task_calls) >= 1, (
            f"Expected OPEN task created on acceptance. Got: {create_task_calls}"
        )
        all_task_text = " ".join(str(c) for c in create_task_calls).upper()
        assert "OPEN" in all_task_text or "REJECTED" not in all_task_text, (
            f"Expected OPEN (not REJECTED) task. Got: {create_task_calls}"
        )
        assert stub.was_called("send_message"), (
            "Expected send_message for Dev Lead hand-off. "
            f"All calls: {[c['tool'] for c in stub.all_calls]}"
        )
        ...
        assert dev_lead_mention, (
            f"Expected [dev lead] in send_message. calls: {send_message_calls}"
        )
        accepted_writes = [
            c for c in create_task_calls
            if "ACCEPTED" in str(c).upper()
        ]
        ...
        assert not reply_with_dev_lead, (
