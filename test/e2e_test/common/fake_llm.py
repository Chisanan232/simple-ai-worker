...
class FakeLLM(BaseLLM):
    ...
    def call(
        ...
        **kwargs: Any,
        ...
        # Use getattr with a default of 0 because threading.local() attributes
        # are NOT shared across threads — a new worker thread (e.g. from
        # ThreadPoolExecutor or testcontainers) will never have 'turn' set
        # yet, so a plain attribute access raises AttributeError.
        current_turn: int = getattr(self._local, "turn", 0) or 0
        if len(messages) <= 2:
            current_turn = 0
    ...
    def _tool_args(
        ...
        if bare in ("add_comment", "add_comment_2"):
            ...
            # CacheHandler (key = "{tool}-{args_json}").  Without this,
            # crews processing cu-31 / cu-32 after cu-30 would hit the cached
            # result from cu-30 and never invoke the stub handler.
            task_id_match = _re.search(r"\bcu-[a-zA-Z0-9]+\b", prompt)
            issue_id_match = _re.search(r"\b([A-Z]+-\d+)\b", prompt)
            ticket_id: str = ""
            if task_id_match:
                ticket_id = task_id_match.group(0)
            elif issue_id_match:
                ticket_id = issue_id_match.group(1)
            ticket_suffix = f" (ticket: {ticket_id})" if ticket_id else ""
            body = (
                "## 📋 Development Plan (v1)\n\n"
                "## Overview\n"
                f"Implementation plan generated by the Dev Agent{ticket_suffix}.\n\n"
                "## Technical Approach\n"
            ...
            if task_id_match:
                base["task_id"] = ticket_id
            elif issue_id_match:
                base["issue_key"] = ticket_id
